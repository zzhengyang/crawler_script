...
class TA_CN:
    def __init__(self):
        self.siteURL = 'http://www.tripadvisor.cn'
    ...
    def getComment(self, page):
        comment = []
        soup = BeautifulSoup(page, 'html.parser')
        commentList = soup.find_all(name="div", attrs={"id": re.compile('review.*?')})
        ...
        for city_id in range(1, 6):
            for category_id in range(1, 6):
                for pageNum in range(0, 300, 30):
                    try:
                        url = "file:///Users/Zyang/Movies/tripadvisor_cn/geo=" + str(
                        ...
                        for href in hrefs:
                            ...
                            try:
                                ...
                                #     Infodict['phone'] = str(shopInfos[2]).replace("电话号码： ", "")
                                #     try:
                                ...
                                # print Infodict
                                # try:
                                #     cur.execute("INSERT INTO merchantsTest(shop_id,name,category_id,coordinate,                 ratings,phone,city_id,avgprice,office_hours,address,bio,      description,photo_urls,content_image_url,ranking,shop_tag) \
                                ...
                                # print "抓取成功第" + str(a) + "条"
                                    ...
                                    for commentItem in commentList:
                                        cur.execute("select id from merchants_cn where shop_id = " + shop_id + ";")
                                        merchant_id = cur.fetchone()
                                        Commentdict['merchant_id'] = int(merchant_id[0])
                                        Commentdict['created_by'] = int('0')
                                        try:
                                            Commentdict['author_name'] = str(
                                                commentItem.find(name="div",
                                                                 attrs={"class", "username mo"}).getText()).replace("\n",
                                                                                                                    "")
                                        except AttributeError, e:
                                            continue
                                        # time.sleep(1.5)
                                        ...
                                        cur.execute("SELECT id FROM comments_cn WHERE author_name='"+Commentdict['author_name']+"'")
                                        results = cur.fetchone()
                                        ...
                                        if results is not None:
                                            ...
                                            try:
                                                cur.execute("INSERT INTO comments_cn(shop_id,merchant_id,created_by,author_name,content) \
                                                             VALUES (%d     , %d        ,%d        ,'%s'       ,'%s'  );" % \
