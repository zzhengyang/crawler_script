...
os.environ['http_proxy'] = "60.21.209.114:8080"
class CQASO:
    def __init__(self):
        ...
        self.proxy={'http':'http://60.21.209.114:8080'}
    # 获得主页面html
...
def start():
    ...
    for rankInfoNum in range(376,len(rankInfoList)):#197-300
        # print rankInfoList[rankInfoNum][1]
        rankInfo = rankInfoList[rankInfoNum]
        appId = int(rankInfo[0])
        # appId = 1148524123
        appName = rankInfo[1].encode('utf-8')
        for day in range(0,265):
            #http://backend.cqaso.com/app/580370312/asoWord?limit=100000&offset=0&pastDate=20160201
            queryURL = "/Users/Zyang/Documents/ASO/CQASO_html/"+str(appId)+"/"+cqaso.getTimeRange(day)['lastYearTime']+".html"
            ...
            cur.execute("SELECT id FROM keyword_html WHERE query_date='%s' and app_id=%d " % (sdate, appId))
            ...
            if results is not None:
                ...
                #     print keywordInfoList
                        ...
                        try:
                            stmt = "INSERT INTO keyword_html(app_id,app_name,app_word,app_rank,app_priority,app_searchcount,app_floatrank,query_date)\
                                                    VALUES (%s,    %s,      %s,       %s,       %s,           %s,               %s,           %s)"
                            cur.executemany(stmt, keywordInfoList)
                            conn.commit()

                            print "appId为" + str(appId) + "的app增加数据"+str(len(keywordInfoList))+"条，时间为"+cqaso.getTimeRange(day)['lastYearTime']+'\r'
                        except:
