...
class ASO100:
    ...
    def downLoadKeyWord(self, appID):
        # 关键词
        ...
        behindURL = "/device/iphone/country/cn/genre/36/ydate/" + self.getTimeRange(1)['lastYearTime'] \
                    + "kdate/" + self.getTimeRange(1)['currentTime']
        self.downLoadFiles(frontURL, appID, behindURL)
    ...
    def start(self):
        ...
        a = 0
        appList = []
        cur.execute("SELECT DISTINCT app_id FROM app_id WHERE category_id=6002")
        ...
        for i in appList:
            cur.execute("SELECT id FROM download WHERE app_id= " + str(i) + "")
            ...
            if results is not None:
                ...
                try:
                    ...
                    cur.execute("INSERT INTO download(app_id) VALUES (%d)" % appID)
                    conn.commit()

                    # rankPage = self.getDetailPage("rank",str(appID))
                    aso.downLoadRank(appID)
                    time.sleep(4)
                    # commentPage = aso.getDetailPage("commentList", appID)
                    aso.downLoadComment(appID)
                    time.sleep(4)
                    # keyWordPage = aso.getDetailPage("keyword", appID)
                    aso.downLoadKeyWord(appID)
                    time.sleep(4)
